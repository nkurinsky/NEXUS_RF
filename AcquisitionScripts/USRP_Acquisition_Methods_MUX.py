...
def generate_daq_params(f_res_GHz, front_end="A", rate=100e6, tx_gain=0.0, rx_gain=17.0, LO_freq=4.25e9, rf_power=-30.0, 
...
def run_vna(series, run_params, res_search_freqs_GHz=None, h5_group_obj=None):
    ...
    ## Loop over a VNA scan for each tone of interest
    for i, f_res_GHz in enumerate(run_params["f_res_GHz"]):
        f_res_Hz = f_res_GHz * 1e9
        ...
        if (run_params["vna"]["f_start_Hz"] is not None) and (run_params["vna"]["f_stop_Hz"] is not None):
            ...
            run_params["vna"]["f_center_Hz"] = f_res_Hz
        ...
        if h5_group_obj is not None:
            gVNA = h5_group_obj.create_group('VNA'+str(i))
        ...
        ## Save the primary/first resonance in the VNA sweep to the daq params dict
        run_params["vna"]["result_fs"] = np.append(run_params["vna"]["result_fs"],fs[0]*1e9) ## Store it in Hz
        run_params["vna"]["result_qs"] = np.append(run_params["vna"]["result_qs"],qs[0])

    ## Extract the important parameters from fit, Get res freqs in Hz (fs is in GHz)
    return -1 # fs*1e9, qs, vna_filename


def run_stream(series, run_params, h5_group_obj=None, run_type="Noise", suffix=None):

    ## Pull the VNA results
    fs = run_params["vna"]["result_fs"]
    qs = run_params["vna"]["result_qs"]

    ## Determine how many equivalent tones to split the DAC power output into to achieve the requested output power
    N_power = np.power(10.,(((-1*run_params["rf_power"])-14)/20.))
    pwr_clc = np.round(-14-20*np.log10(N_power),2)

    ## Make array of the central tone and tracking tones (e.g.: [fres0, fres1, ..., fresN, fTa, fTb, ..., fTz])
    readout_tones  = np.append(fs, run_params["stream"]["track_tones_Hz"])
